...
describe("RBAC (Role-Based Access Control) Test", async () => {
	...
	it("should get role admin", async () => {
		...
		assert.equal(
			roleAdmin,
			ROLE_OWNER,
			"Owner should be admin of manager role",
		);
	});
	...
	it("should be able to revoke roles", async () => {
		...
		assert.equal(
			hasRole,
			false,
			"Other wallet should not have manager role anymore",
		);
	});
	...
	it("only owner should be able to perform diamond cuts", async () => {
		...
		const selectors = getSelectors(testFacet).remove([
		...
		assert.equal(
			result,
			parseEther("1"),
			"Test facet should be properly added",
		);
	});
